...
const InvoicePage: FC<Props> = ({ data, pdfMode, onChange }) => {
  ...
  const [cgstTax, setCgstTax] = useState<number>()
  const [sgstTax, setSgstTax] = useState<number>()
  ...
  useEffect(() => {
    const match = invoice?.cgstLabel.match(/(\d+)%/)
    const taxRate = match ? parseFloat(match[1]) : 0
    const cgstTax = subTotal ? (subTotal * taxRate) / 100 : 0

    setCgstTax(cgstTax)
  }, [subTotal, invoice.cgstLabel])
  useEffect(() => {
    const match = invoice.sgstLabel.match(/(\d+)%/)
    const sgstRate = match ? parseFloat(match[1]) : 0
    const sgstTax = subTotal ? (subTotal * sgstRate) / 100 : 0

    setSgstTax(sgstTax)
  }, [subTotal, invoice.sgstLabel])

  useEffect(() => {
    const match = invoice.discountLabel.match(/(\d+)%/)
    const discountRate = match ? parseFloat(match[1]) : 0
    console.log("-------------discount--------", discount);
    const discountFinal = subTotal ? (subTotal * discountRate) / 100 : 0
    console.log("discountFinal", discountFinal);
    setDiscount(discountFinal)
            ...
            <View className="flex bg-gray p-5" pdfMode={pdfMode}>
              ...
              <View className="w-50 p-5 flex" pdfMode={pdfMode}>
                ...
                <Text className="right bold dark w-auto" pdfMode={pdfMode}>
                  {(typeof subTotal !== 'undefined' && typeof cgstTax !== 'undefined'
                    && typeof sgstTax !== 'undefined' && typeof discount !== 'undefined'
                    ? subTotal + cgstTax + sgstTax - discount
                    : 0
                  ).toFixed(2)}
                </Text>
              </View>
